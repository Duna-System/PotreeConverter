...
PotreeArguments parseArguments(int argc, char **argv) {
  ...
  args.addArgument("spacing-by-diagonal-fraction,d",
                   "Maximum number of points on the diagonal in the first level (sets spacing). "
                   "spacing = diagonal / value");
  args.addArgument(
      "levels,l",
      "Number of levels that will be generated. 0: only root, 1: root and its children, ...");
  args.addArgument("input-format,f",
                   "Input format. xyz: cartesian coordinates as floats, rgb: colors as numbers, i: "
                   "intensity as number");
  ...
  args.addArgument("output-attributes,a",
                   "can be any combination of RGB, INTENSITY and CLASSIFICATION. Default is RGB.");
  args.addArgument("scale", "Scale of the X, Y, Z coordinate in LAS and LAZ files.");
  args.addArgument("aabb",
                   "Bounding cube as \"minX minY minZ maxX maxY maxZ\". If not provided it is "
  ...
  args.addArgument("material",
                   "RGB, ELEVATION, INTENSITY, INTENSITY_GRADIENT, CLASSIFICATION, RETURN_NUMBER, "
  ...
  if (args.has("aabb")) {
    ...
    for (size_t p = 0, q = 0; p != strAABB.npos; p = q)
      aabbValues.push_back(atof(
          strAABB.substr(p + (p != 0), (q = strAABB.find(sep, p + 1)) - p - (p != 0)).c_str()));
  ...
  vector<string> validMaterialNames = {
      "RGB",           "ELEVATION", "INTENSITY",      "INTENSITY_GRADIENT", "CLASSIFICATION",
      "RETURN_NUMBER", "SOURCE",    "LEVEL_OF_DETAIL"};
  if (std::find(validMaterialNames.begin(), validMaterialNames.end(), a.material) ==
      validMaterialNames.end()) {
    cout << args.usage();
    cout << endl;
    cout << "ERROR: "
         << "invalid material name specified" << endl;
    exit(1);
  }

  // set default parameters
  fs::path pSource(a.source[0]);
  a.outdir = args.has("outdir") ? args.get("outdir").as<string>()
                                : pSource.generic_string() + "_converted";
