...
void PotreeConverter::generatePage(string name) {
  ...
  if (!this->sourceListingOnly) {  // change viewer template
    ...
    while (getline(in, line)) {
      if (line.find("<!-- INCLUDE POINTCLOUD -->") != string::npos) {
        out << "\t\tPotree.loadPointCloud(\"pointclouds/" << name << "/cloud.js\", \"" << name
            << "\", e => {" << endl;
        ...
        if (showSkybox) {
          out << "\t\t"
              << "viewer.setBackground(\"skybox\"); // [\"skybox\", \"gradient\", \"black\", "
                 "\"white\"];\n";
        } else {
          out << "\t\t"
  ...
  //	ssSettings << "\tcameraPosition: null, 		// other options: cameraPosition:
  //[10,10,10]," << endl; 	ssSettings << "\tcameraTarget: null, 		// other options:
  // cameraTarget: [0,0,0]," << endl;
  //	ssSettings << "\tfov: 60, 					// field of view in degrees,"
  //<< endl; 	ssSettings << "\tsizeType: \"Adaptive\",	// other options: \"Fixed\",
  // \"Attenuated\""
  //<< endl; 	ssSettings << "\tquality: null, 			// other options:
  //\"Circles\",
  //\"Interpolation\", \"Splats\"" << endl;
  //	ssSettings << "\tmaterial: \"RGB\", 		// other options: \"Height\", \"Intensity\",
  //\"Classification\"" << endl;
  //	ssSettings << "\tpointLimit: 1,				// max number of points in millions"
  //<< endl; 	ssSettings << "\tpointSize: 1,				// " << endl; 	ssSettings
  // <<
  //"\tnavigation: \"Orbit\",		// other options: \"Orbit\", \"Flight\"" << endl;
  //ssSettings
  //<< "\tuseEDL: false,				" << endl; 	ssSettings << "};" << endl;
  //
  //
  //	ofstream fSettings;
...
void writeSources(string path, vector<string> sourceFilenames, vector<int> numPoints,
                  vector<AABB> boundingBoxes, string projection) {
...
void PotreeConverter::convert() {
  ...
  if (fs::exists(fs::path(this->workDir + "/cloud.js"))) {
    if (storeOption == StoreOption::ABORT_IF_EXISTS) {
      ...
      writer = new PotreeWriter(this->workDir, aabb, spacing, maxDepth, scale, outputFormat,
