...
float PWNode::spacing() { return float(potreeWriter->spacing / pow(2.0, float(level))); }
...
PointReader *PWNode::createReader(string path) {
  ...
  if (outputFormat == OutputFormat::LAS || outputFormat == OutputFormat::LAZ) {
    ...
    reader =
        new BINPointReader(path, aabb, potreeWriter->scale, this->potreeWriter->pointAttributes);
  }
...
PointWriter *PWNode::createWriter(string path) {
  ...
  if (outputFormat == OutputFormat::LAS || outputFormat == OutputFormat::LAZ) {
    ...
    writer =
        new BINPointWriter(path, aabb, potreeWriter->scale, this->potreeWriter->pointAttributes);
  }
...
PWNode *PWNode::add(Point &point) {
  ...
  if (isLeafNode()) {
    ...
    //		accepted = accepted && node->grid->willBeAccepted(point.position,
    // grid->squaredSpacing); 		node = node->parent;
    //	}
    //
    //	//node = this;
...
void PWNode::flush() {
  std::function<void(vector<Point> & points, bool append)> writeToDisk = [&](vector<Point> &points,
                                                                             bool append) {
    string filepath = workDir() + "/data/" + path();
    PointWriter *writer = NULL;
...
PotreeWriter::PotreeWriter(string workDir, AABB aabb, float spacing, int maxDepth, double scale,
                           OutputFormat outputFormat, PointAttributes pointAttributes,
...
void PotreeWriter::setProjection(string projection) { this->projection = projection; }
