      ...
      while (true) {
        ...
        if (4 == split.size() || 7 == split.size()) {
          ...
          if (0.5 != intensity) {
            ...
            count++;
            if (0 == count % 1000000)
              cout << "AABB-SCANNING: " << count << " points; " << currentChunk << " chunks"
                   << endl;
          }
        } else {
          break;
...
bool PTXPointReader::loadChunk(fstream *stream, long currentChunk, double tr[16]) {
  ...
  if (!assertd(*stream, 1) || !assertd(*stream, 3) || !assertd(*stream, 3) ||
      !assertd(*stream, 3) || !assertd(*stream, 3))
    return false;
...
bool PTXPointReader::readNextPoint() {
  while (true) {
    ...
    if (INVALID_INTENSITY != p.intensity) return true;
  }
  return false;
}
