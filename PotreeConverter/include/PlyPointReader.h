...
struct PlyPropertyType {
  ...
  PlyPropertyType(string name, int size) : name(name), size(size) {}
};
...
struct PlyProperty {
  ...
  PlyProperty(string name, PlyPropertyType type) : name(name), type(type) {}
};
...
struct PlyElement {
  ...
  PlyElement(string name) : name(name) {}
};

unordered_map<string, PlyPropertyType> plyPropertyTypes = {
    {"char", PlyPropertyType("char", 1)},     {"int8", PlyPropertyType("char", 1)},
    {"uchar", PlyPropertyType("uchar", 1)},   {"uint8", PlyPropertyType("uchar", 1)},
    {"short", PlyPropertyType("short", 2)},   {"int16", PlyPropertyType("short", 2)},
    {"ushort", PlyPropertyType("ushort", 2)}, {"uint16", PlyPropertyType("ushort", 2)},
    {"int", PlyPropertyType("int", 4)},       {"int32", PlyPropertyType("int", 4)},
    {"uint", PlyPropertyType("uint", 4)},     {"uint32", PlyPropertyType("uint", 4)},
    {"float", PlyPropertyType("float", 4)},   {"float32", PlyPropertyType("float", 4)},
    {"double", PlyPropertyType("double", 8)}, {"float64", PlyPropertyType("double", 8)}};
...
class PlyPointReader : public PointReader {
 ...
 public:
  PlyPointReader(string file)
      : stream(file, std::ios::in | std::ios::binary), vertexElement("vertexElement") {
    format = -1;
    pointCount = 0;
    pointsRead = 0;
    pointByteSize = 0;
    ...
    std::regex rProperty(
        "^property "
        "(char|int8|uchar|uint8|short|int16|ushort|uint16|int|int32|uint|uint32|float|float32|"
        "double|float64) (\\w*)");
    ...
    if (format == PLY_FILE_FORMAT_ASCII) {
      ...
      for (const auto &prop : vertexElement.properties) {
        ...
        if (prop.name == "x" && prop.type.name == plyPropertyTypes["float"].name) {
          ...
          z = stod(token);
        } else if (std::find(plyRedNames.begin(), plyRedNames.end(), prop.name) !=
                       plyRedNames.end() &&
                   prop.type.name == plyPropertyTypes["uchar"].name) {
          r = (unsigned char)stof(token);
        } else if (std::find(plyGreenNames.begin(), plyGreenNames.end(), prop.name) !=
                       plyGreenNames.end() &&
                   prop.type.name == plyPropertyTypes["uchar"].name) {
          g = (unsigned char)stof(token);
        } else if (std::find(plyBlueNames.begin(), plyBlueNames.end(), prop.name) !=
                       plyBlueNames.end() &&
                   prop.type.name == plyPropertyTypes["uchar"].name) {
  ...
  long long numPoints() { return pointCount; }
