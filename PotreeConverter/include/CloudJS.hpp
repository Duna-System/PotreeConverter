...
class CloudJS {
 public:
  ...
  CloudJS(string content) {
    ...
    boundingBox =
        AABB(Vector3<double>(vBoundingBox["lx"].GetDouble(), vBoundingBox["ly"].GetDouble(),
                             vBoundingBox["lz"].GetDouble()),
             Vector3<double>(vBoundingBox["ux"].GetDouble(), vBoundingBox["uy"].GetDouble(),
                             vBoundingBox["uz"].GetDouble()));
    tightBoundingBox = AABB(
        Vector3<double>(vTightBoundingBox["lx"].GetDouble(), vTightBoundingBox["ly"].GetDouble(),
                        vTightBoundingBox["lz"].GetDouble()),
        Vector3<double>(vTightBoundingBox["ux"].GetDouble(), vTightBoundingBox["uy"].GetDouble(),
                        vTightBoundingBox["uz"].GetDouble()));

    if (vPointAttributes.IsArray()) {
      outputFormat = OutputFormat::BINARY;
      pointAttributes = PointAttributes();

      for (Value::ConstValueIterator itr = vPointAttributes.Begin(); itr != vPointAttributes.End();
           ++itr) {
