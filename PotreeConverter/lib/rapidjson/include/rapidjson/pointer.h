...
class GenericPointer {
  ...
  */
  GenericPointer Append(SizeType index, Allocator* allocator = 0) const {
    char buffer[21];
    SizeType length = (sizeof(SizeType) == 4 ? internal::u32toa(index, buffer)
  ...
  */
      ...
      if (v->IsArray() && t->name[0] == '-' && t->length == 1) {
        ...
        if (v->IsArray()) {
          if (t->index >= v->Size()) {
            ...
            while (t->index >= v->Size()) v->PushBack(Value().Move(), allocator);
            exist = false;
          }
          v = &((*v)[t->index]);
        } else {
  ...
  /*!
     ...
     not exist. \param alreadyExist If non-null, it stores whether the resolved value is already
  ...
  ValueType* Get(ValueType& root) const {
    ...
    for (const Token* t = tokens_; t != tokens_ + tokenCount_; ++t) {
      switch (v->GetType()) {
        ...
        case kArrayType:
          if (t->index == kPointerInvalidIndex || t->index >= v->Size()) return 0;
          v = &((*v)[t->index]);
          break;
        default:
          return 0;
  ...
  */
  ...
  /*!
     ...
     Representation of JSON Pointer, e.g. In "/\u0000", \u0000 will not be unescaped.
  */
