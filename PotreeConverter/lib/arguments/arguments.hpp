...
class AValue {
 public:
  ...
  AValue(vector<string> values) { this->values = values; }
...
template <>
vector<string> AValue::as<vector<string>>(vector<string> alternative) {
  return !values.empty() ? values : alternative;
}

template <>
vector<string> AValue::as<vector<string>>() {
  return !values.empty() ? values : vector<string>{};
}

template <>
vector<double> AValue::as<vector<double>>(vector<double> alternative) {
  vector<double> res;
  for (auto &v : values) {
...
template <>
vector<double> AValue::as<vector<double>>() {
